...
.ai-response { background:rgba(20,38,55,0.92); border:1px solid #0288d1; border-radius:16px;
    padding:1.5rem; margin:1rem 0; color:#e8f5e9; line-height:1.8; }
...
Farmer in {city}, {country} wants to grow {crop}.
Conditions: Temperature={temp}°C, Rainfall={rainfall:.0f}mm/year, Soil moisture={soil:.2f}
Predicted Yield: {pred:,.0f} kg/ha | Historical Average: {avg:,.0f} kg/ha
Provide: 1) Crop suitability analysis 2) Factors affecting yield 3) Recommendations to improve yield 4) Risks 5) Alternative crops.
...
st.markdown(”<div class='info-box'>💡 Powered by Google Gemini AI — Get personalized agricultural recommendations.</div>”, unsafe_allow_html=True)
st.markdown(”<br>”, unsafe_allow_html=True)
if st.button(“🤖 Get AI Agricultural Advice”):
if hasattr(st.session_state, ‘prediction’):
with st.spinner(“🤖 Analysing your conditions…”):
advice = get_ai_advice(
st.session_state.get(‘pred_country’, selected_country),
st.session_state.get(‘pred_city’,’’) or selected_country,
st.session_state.get(‘pred_crop’, selected_crop),
st.session_state.temperature, st.session_state.rainfall,
soil_moisture, st.session_state.prediction, st.session_state.avg_yield)
...
colors = ['#69f0ae','#40c4ff','#ffeb3b','#ff6e40','#ea80fc','#b9f6ca','#ffab40']
cl = dict(paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)',font={'color':'#e8f5e9'})
fig = go.Figure(go.Bar(x=importances,y=features,orientation='h',marker=dict(color=colors),
